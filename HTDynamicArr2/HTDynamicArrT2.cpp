...
void Task2(const int* arr1, const int* arr2, int* arr3, size_t size1, size_t size2, size_t* counter)
{
	for (int i = 0; i < size1; ++i)
	{
		arr3[*counter] = arr1[i];
		++(*counter);						//не помню точно приоритет операций, потому занесу указатель в скобки.
		for (int j = 0; j < size2; ++j)
		{
			if (arr1[i] == arr2[j])
			{
...
{
	int choose;
	size_t size1, size2, size3, counter = 0;
	srand(time(0));
	...
	size3 = sizeCheck(arr1, arr2, size1, size2) + sizeCheck(arr2, arr1, size2, size1);
	if (!size3)
	{
	...
	Task2(arr1, arr2, arr3, size1, size2, &counter);
	Task2(arr2, arr1, arr3, size2, size1, &counter);
